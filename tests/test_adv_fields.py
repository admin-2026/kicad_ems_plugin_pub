...
board, no dialog -- so they are checked here rather than through a live pane.
...
def test_every_advanced_field_is_a_writer_knob():
    ...
    for key, label, _hint in options.ADV_FIELDS:
        assert key in config.DEFAULTS, f"{label} ({key}) is not a config knob"
...
def test_every_speed_preset_knob_is_a_writer_knob():
    ...
    for name in options.SpeedPreset._fields:
        if name == "desc":
            continue
        assert name in config.DEFAULTS, f"{name} is not a config knob"
...
def test_negatives_are_refused_naming_the_field():
    ...
    for key in ("min_raster_mm", "mesh_budget_gb", "auto_ground_radius_mm", "cell_mm"):
        try:
            advanced._adv_value(key, "Field", "-1")
...
def test_zero_is_the_auto_for_every_field():
    ...
    assert advanced._adv_value("min_raster_mm", "Field", "0") == 0.0
    assert advanced._adv_value("mesh_budget_gb", "Field", "0") == 0.0
    assert advanced._adv_value("pml_cells", "Field", "0") == 0
