...
checkout has no remote to read it from -- so ``REPO`` is written below, as the
public repository the releases are published from (the same one
``links.GITHUB_URL`` offers on the About page; the two move together). Left
...
what turns them into an Outcome.
"""
...
_HEADERS = {
    ...
    "User-Agent": "AntennaDesigner-KiCad-plugin",
}
