...
def arrow_segments(cx, cy, half_w, tri_h):
    ...
    ``(cx, cy)`` and its apex ``tri_h`` toward −Y, plus a stem running the
    other way (+Y) ``half_w`` long — half the base width. The base is drawn as
    two halves meeting at the centre so the stem shares that vertex. Both
    markers build their feed from this and decode_arrow reads it back, so the
    drawn shape and its meaning have one definition. arrow_dot puts a dot on
    that base centre, the feed point the whole arrow is about."""
    c = (cx, cy)
    base_l, base_r = (cx - half_w, cy), (cx + half_w, cy)
    apex = (cx, cy - tri_h)
    tail = (cx, cy + half_w)  # stem length = half the base width
