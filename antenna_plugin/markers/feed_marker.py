...
def _shape_type_id(new_name, old_name):
...
def _new_shape(fp, shape_t):
    ...
    not yet added to it. The class is FP_SHAPE on KiCad 6/7 and PCB_SHAPE from
    8 on."""
    import pcbnew
...
def _add_segment(fp, layer_id, seg_mm, stroke_mm):
    ...
    currently sits in -- board coordinates for a placed footprint (the
    wizard's antenna preview, area_marker.draw_antenna), footprint-local for
    one still being built at origin (_build_footprint). Shared by both, the
    KiCad-6 FP_SHAPE local-coordinate sync included."""
    import pcbnew

    from ..kicad.compat import vec2

    (x0, y0), (x1, y1) = seg_mm
    seg = _new_shape(fp, _shape_type_id("SHAPE_T_SEGMENT", "S_SEGMENT"))
    seg.SetStart(vec2(pcbnew.FromMM(x0), pcbnew.FromMM(y0)))
    seg.SetEnd(vec2(pcbnew.FromMM(x1), pcbnew.FromMM(y1)))
    seg.SetLayer(layer_id)
    seg.SetWidth(pcbnew.FromMM(stroke_mm))
...
def _add_filled_circle(fp, layer_id, center_mm, radius_mm):
    ...
    exactly ``radius_mm``."""
    circle = _new_shape(fp, _shape_type_id("SHAPE_T_CIRCLE", "S_CIRCLE"))
...
def _rewrite_filled_circle(circle, layer_id, center_mm, radius_mm):
    ...
    as an end point ``radius_mm`` to the right of the centre; rotation of the
    placed footprint leaves both correct."""
    import pcbnew

    from ..kicad.compat import vec2

    (x, y) = center_mm
    circle.SetStart(vec2(pcbnew.FromMM(x), pcbnew.FromMM(y)))
    circle.SetEnd(vec2(pcbnew.FromMM(x + radius_mm), pcbnew.FromMM(y)))
    circle.SetLayer(layer_id)
    circle.SetFilled(True)
    circle.SetWidth(0)
    if hasattr(circle, "SetLocalCoord"):  # KiCad 6 FP_SHAPE keeps local pts
...
def _add_filled_poly(fp, layer_id, points_mm):
    ...
    the wizard's antenna preview, area_marker.draw_antenna). The outline
    stroke is zero-width so only the fill shows."""
    poly = _new_shape(fp, _shape_type_id("SHAPE_T_POLYGON", "S_POLYGON"))
...
def _rewrite_filled_poly(poly, layer_id, points_mm):
    ...
    for another candidate (area_marker.draw_antenna).
    ...
    (see _detach_item). Reusing a shape keeps every pointer to it valid."""
    import pcbnew
    ...
    poly.SetLayer(layer_id)
    poly.SetFilled(True)
    poly.SetWidth(0)
    if hasattr(poly, "SetLocalCoord"):  # KiCad 6 FP_SHAPE keeps local pts
        poly.SetLocalCoord()
    return poly
...
def _detach_item(fp, item):
    """Take one graphic off a placed marker footprint, leaking the C++ object
    rather than letting it be freed — the only safe way for a plugin to drop a
    graphic while the PCB editor is live.
    ...
    saved inside the area marker — the editor's view, the connectivity graph,
    ...
    callers reuse what they can instead — ``area_marker.draw_antenna`` rewrites
    every preview shape it can (_rewrite_filled_poly) and only comes here for
    the surplus a shorter candidate leaves behind."""
    fp.Remove(item)
    try:
        item.thisown = False
...
def _segment_items(fp):
    ...
    return [
        item
        for item in _shape_items(fp, _shape_type_id("SHAPE_T_SEGMENT", "S_SEGMENT"))
        if item.GetLayer() in user_ids
    ]


def _filled_poly_items(fp):
    """A marker footprint's filled-polygon graphics, in draw order -- the
    wizard's antenna preview (area_marker.antenna_items), and nothing else the
    plugin draws on a marker.

    Selected by shape rather than by layer, because the preview moves between
    layers: copper while the candidate fits the area, the marker's own User
    layer while it doesn't (area_marker.draw_antenna). The marker's own drawing
    is line segments on a User layer, so the two never collide whichever layer
    the preview is on."""
    return _shape_items(fp, _shape_type_id("SHAPE_T_POLYGON", "S_POLYGON"))
...
def _circle_items(fp):
    ...
    return _shape_items(fp, _shape_type_id("SHAPE_T_CIRCLE", "S_CIRCLE"))
