...
emkit.markers.feed_marker footprint on the cursor for the user to click onto
...
(sections.area.AreaSection) is the same idea — the plugin's own drawing on a
User layer, placed once and never replaced — so it subclasses this section and
reuses those helpers, its own ``_build`` replacing the feed-specific UI and its
own ``_apply_live`` / ``refresh_status`` writing and describing its own marker.
Where they part: this marker is a footprint the sliders resize, while the area
marker is a group of board shapes the *user* drags into shape, so its
``_apply_live`` follows the board instead of driving it. ``placed_markers`` is
the seam ``_markers`` finds either kind through.
"""
...
class FeedMarkerSection(Section):
    ...
    # this one is resized.
    # How many of this marker a run uses. One port for an antenna; a flow with
    # more says so, and the button stops placing once the board has that many.
    MARKER_LIMIT = 1

    # How the extras nudge finishes, when the board has more markers than the
    # run has ports for.
    _NEEDS = "the run needs exactly one"

    _ALREADY_HINT = (
        "the sliders resize it, or delete it (Del) to re-place it from the cursor."
    )
    ...
    def _markers(self):
        """The placed markers of this section's ``marker_module`` (empty with
        no board). ``placed_markers`` is the seam between the two: the feed
        marker's are footprints, the area marker's are groups of board shapes
        (so KiCad will let the user drag them)."""
        import pcbnew

        board = pcbnew.GetBoard()
        return self.marker_module.placed_markers(board) if board is not None else []
    ...
    # written from (emkit.settings). So the two markers share a User layer:
    ...
    def feed_sync_labels(self):
        ...
        Both marker sections have exactly this one slider to sync."""
    ...
    def marker_text(self, fp):
        """One placed marker as a sentence: where it is, on which layer, and
        the width its drawing decodes to."""
        d = feed_marker.describe_marker(fp)
        text = (
    ...
    def _placed_text(self, fps):
        """The placed-marker status: the markers the run will use, plus a
        cleanup nudge when the board carries more than it has room for."""
        text = "; ".join(self.marker_text(fp) for fp in fps[: self.MARKER_LIMIT])
        if len(fps) > self.MARKER_LIMIT:
            text += (
                f"; {len(fps)} markers found — delete the extras (Del), {self._NEEDS}"
            )
        return text + "."
    ...
    def _reveal(self, fp):
        ...
        try:
            board = pcbnew.GetBoard()
            layer_id = self.marker_module.marker_layer_id(fp)
    ...
    def on_place_marker(self, event=None):
        ...
        try:
            ...
            existing = self._markers()
            if len(existing) >= self.MARKER_LIMIT:
