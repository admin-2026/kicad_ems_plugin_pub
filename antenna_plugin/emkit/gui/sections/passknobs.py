...
A run (sections.run) and a sweep both hand the solver two decisions that are
about the pass rather than about the board being designed:
...
None of these is a :class:`~emkit.gui.sections.base.Section` -- they
...
four over every row it holds, so a page names its knobs once
(``pages.designform.DesignFormPage.pass_knobs``) and a third knob is added here
rather than in both sections.

A row owns the widgets and what they snapshot to; it does not own what its
value *means* to a run. That is one translation for both frontends
(``formparams.pass_knobs``, over the same flat form a settings file holds), so
``contribute`` is PassKnobs' alone and no row has one.
"""
...
from ..widgets import bool_str, enable, muted, parse_bool, set_tip
...
class PassKnobs:
    ...
    def contribute(self, params):
        """Add this pass' knobs to a run's params, through the form both
        frontends read (formparams.pass_knobs): the rows say what the form
        holds, and what it means is said once, where a settings file is read
        too."""
        formparams.pass_knobs(self.snapshot(), params)
