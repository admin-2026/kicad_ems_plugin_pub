"""SolverSection: the shared base for the sections that drive the solver.
...
already in flight) only when something else *is*, and the user decides.

The list of live passes is **a directory of files beside the board**
(``sim.runlock``), not a list in this process. It used to be the latter, which
was safe for exactly as long as a window was the only thing that could start a
solver; a run started from the command line and a run started from here write
the same ``simulation/`` folder and have to be able to see each other. What
stays here is the conversation -- a dialog needs somebody to have it with --
and the ``_running`` flag the buttons read. ``_claim`` / ``_release`` own both
together, so what a section believes about itself and what the board's folder
...
class SolverSection(Section):
    ...
    def __init__(self, page, step=None):
        ...
        self._claim_entry = None  # this pass' file in the board's lock dir
    ...
    # --- one pass, or several -------------------------------------------------
    def _sim_dir(self):
        """This board's ``simulation`` folder, where the claims live. None
        when there is no board open, or none saved to a file yet -- then there
        is nowhere to claim and nothing to warn about, which is the same
        answer as "nothing is running". (A pass that really needs the file
        refuses a moment later, by name: ``_ensure_board_saved``.)"""
        import pcbnew
        ...
        try:
            ...
            return None if board is None else str(simulate.output_dir(board))
        except Exception:
            return None
    ...
    def confirm_concurrent(self):
        ...
        which is what the warning is about. What counts as "already in flight"
        is every process' passes, not this one's (sim.runlock)."""
        sim_dir = self._sim_dir()
        if sim_dir is None:
            return True
        mine = (self._claim_entry.path,) if self._claim_entry else ()
        others = runlock.labels(sim_dir, exclude=mine)
    ...
    def _claim(self):
        """This section's pass is starting: flag it running and leave a claim
        in the board's folder, which is what every other pass -- in this
        window or in another process -- reads."""
        self._running = True
        sim_dir = self._sim_dir()
        if sim_dir is not None and self._claim_entry is None:
            self._claim_entry = runlock.claim(sim_dir, self.busy_label)

    def _release(self):
        """The pass is over, however it ended: clear the flag and drop the
        claim. Called from every finish path -- and from ``shutdown``, so a
        window closed mid-run doesn't leave a pass behind that nothing will
        ever end. (A window that dies without getting here leaves a claim
        whose pid is gone, which the next reader sweeps up.)"""
        self._running = False
        runlock.release(self._claim_entry)
        self._claim_entry = None
    ...
    def _report_failure(self, message):
        ...
        dlg = wx.MessageDialog(
            self.page, message, product.NAME, wx.OK | wx.ICON_WARNING
        )
        dlg.ShowModal()
        dlg.Destroy()
