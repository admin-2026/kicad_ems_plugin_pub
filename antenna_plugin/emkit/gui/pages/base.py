...
Subclass contract:
  ...
  * override ``settings_snapshot`` / ``settings_restore`` to take part in the
    per-project settings file (settings), and call ``load_settings()`` once
...
class BookPage(wx.Panel):
    ...
    def mount(self, body, footer=None):
        ...
        The scroll is also where macOS needs its sliders repainted after the
        form moves under them (gui.mac_scroll); the call binds nothing on any
        other platform."""
