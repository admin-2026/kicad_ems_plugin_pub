...
// (the substrate slabs and coatings top to bottom, then the foils top to
// bottom, then the drills).
const SECTIONS = [
  ['board', 'On the board'],
...
// Both flows draw this page, and each ships from its own plugin: credit the
// one whose dump was read (scattering_grid.js, or the monopole's pcb_grid.js).
{
  const p = /scattering/.test(window.FDTD.dump || '') ? 'si' : 'ems';
  const a = document.querySelector('.credit a');
  a.textContent = `kicad_${p}_plugin`;
  a.href = `https://github.com/admin-2026/kicad_${p}_plugin_pub`;
}
...
// volume rather than tracing a feature wants far less of it. `span`
// ([z0, z1] in metres, the dump's "zspan") is the z the group's material
// really fills where its cells are not it -- a copper foil is 35 um of metal
// indexed by the substrate cell its plane falls in -- and replaces the cell's
// z extent outright, at its true size: the thickness IS what it is saying, so
// it does not take the 0.92 the in-plane sizes do.
function addCells(cells, color, sheet, opacity = 0.45, span) {
  ...
  const spanZ = span ? 0.5 * (span[0] + span[1]) * s - C[2] : 0;
  const spanH = span ? (span[1] - span[0]) * s : 0;
  for (let q = 0; q < n; q++) {
    const [i, j, k] = [cells[3 * q], cells[3 * q + 1], cells[3 * q + 2]];
    pos.set(mid(0, i), mid(1, j), span ? spanZ : mid(2, k));
    scl.set(wid(0, i) * 0.92, wid(1, j) * 0.92,
            span ? spanH : wid(2, k) * 0.92);
...
for (const g of DATA.groups) {
  ...
  entries.push({label: g.label, color, count: g.cells.length / 3,
                ...
                section: g.section,
                objects: addCells(g.cells, color, g.sheet, opacity, g.zspan)});
}
...
// Anything spliced into this page beyond the template itself: an overlay
// drawing a payload section of its own, or code the page has never heard of
// that arrived with the run (`?with=`, js/dump.js). Each fragment pushes one
// function onto window.FDTD_OVERLAYS as it loads, and this is the one place
// the page calls them -- so adding an overlay is adding a fragment, never
// another guarded call site here, and a page that carries none iterates an
// empty list. It runs before the panel below so an overlay's groups get their
// checkbox like any other.
...
FDTDViewer.boot({
  data: ['pcb_grid.js', 'scattering_grid.js', 'grid.js'],
  main: vizGridPage,
});
