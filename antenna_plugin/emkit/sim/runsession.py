...
The GUI needs three things from a solve beyond "run it and log the lines":
  ...
  * the same for the *grid* dump, which a run writes once, on its way from the
    meshing to the stepping (``Wrote <dir>/<stem>_grid.js``). It is the one
    line that says the lattice is built and the fields are about to start, so
    it is both when the grid can be archived and when the run changes phase;
  * a request that lands between two solver processes (a grid-only pass and a
    later solve) not to be lost or applied to the wrong one.
...
Typical use (from a worker thread)::
    ...
    session.run(exe, yaml_path)                     # mesh, then solve
"""
...
# And what it prints once the grid dump is on disk ("Wrote <path>.js",
# PcbGridView.cpp). Matched by shape rather than by name so this stays free of
# the product's dump stems -- the outputs line above is the only other "Wrote"
# the runners print, and it does not end in .js.
_GRID_MARK = "Wrote "


class RunSession:
    """One solver invocation (or a sequence of them, as a scan's per-candidate
    solves). ``on_line(text)`` sees every stdout line; ``on_outputs(sample)``
    fires once the solver has written a set of outputs, with ``sample`` True
    for a mid-run snapshot and False for a run's final results. ``on_grid()``
    fires once its grid dump is written -- a plain attribute, because it is
    :mod:`sim.runner` that wants it and the frontends are what construct the
    session. All are called on the thread driving :meth:`run`."""

    def __init__(self, on_line=None, on_outputs=None, runner=None, on_grid=None):
    ...
    def _line(self, text):
        ...
        elif stripped.startswith(_GRID_MARK) and stripped.endswith(".js"):
            if self.on_grid:
                self.on_grid()
