"""Static choice tables for the dialog.

Metal models, the speed/accuracy presets and the Advanced pane's fields, sorted
into the functional groups it draws them in (ADV_GROUPS). Pure data — no wx, no
pcbnew. The high-level application picks (and the antenna spec each implies)
live in their own ``applications`` catalog, mirroring the ``materials`` one.
"""
...
# Metal model picker: how the conductor is represented in the FDTD grid.
MODELS = [
    ("sheet", "sheet — fast, lossless"),
    ("sibc", "sibc — skin-effect loss"),
    ("slab", "slab — volumetric foil"),
]
...
ADV_GROUPS = {
    # The per-layer metal/substrate pickers (materials.ui, a box of its own)
    # plus the metal model they are meshed with -- no numeric fields.
    ...
    "mesh": AdvGroup(
        "Mesh",
        (
            ("substrate_cell_mm", "Substrate cell height (mm)", "0 = auto"),
    ...
    "feed": AdvGroup(
        "Feed port",
        (("port_resistance", "Port resistance (Ω)", "50"),),
    ),
...
# collect_run_params passes to the writer (the grouping is a layout fact).
ADV_FIELDS = [field for group in ADV_GROUPS.values() for field in group.fields]

GROUND_CHECKS = ["strict", "warn", "off"]
