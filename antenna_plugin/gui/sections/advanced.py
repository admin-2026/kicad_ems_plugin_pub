...
this module builds one method per box and decides their order):
    ...
    Frequencies and run  the excited band, the steps and the threads
    ...
    Markers              the User layer the marker footprints live on
...
from ..options import ADV_FIELDS, ADV_GROUPS, GROUND_CHECKS, MODELS, SPEED_PRESETS
from ..widgets import (
    FIELD_W,
    bool_str,
    live_text,
...
def _adv_value(key, label, text):
    """One filled-in Advanced field, parsed strictly. Raises naming the
    field's label instead of letting config's float()/int() coercion fail
    with a bare 'invalid literal' message."""
...
class AdvancedSection(Section):
    ...
    def _materials_box(self, pane):
        """Materials: the optional per-layer metal/substrate pickers and the
        model the metal is meshed with. The pickers come from a guarded import
        so the plugin still runs if the materials package is removed -- the box
        then holds the metal model alone (and config's own per-layer sources
        block the run, see contribute)."""
        self.materials = make_material_selector(pane, self.page.form.update_hint)
        box = (
        ...
        grid = _field_grid()
        grid.Add(
            theme.key_label(pane, "Metal model", "copper_model"),
            0,
            wx.ALIGN_CENTER_VERTICAL,
        )
        self.model = wx.Choice(pane, size=(FIELD_W, -1), choices=[d for _, d in MODELS])
        self.model.SetSelection(1)  # sibc (skin-effect loss)
        grid.Add(self.model, 0)
    ...
    def _run_box(self, pane):
        """Frequencies and run: the band the port is excited over, and how long
        the stepping goes on for (the simulation time itself is on the section
        that starts the pass -- Run, or a designer's Scan -- so these are the
        fixed-step and thread overrides beside it)."""
    ...
    def _ground_box(self, pane):
        ...
        grid = _field_grid()
        grid.Add(
            theme.key_label(pane, "Ground check", "ground_check"),
            0,
            wx.ALIGN_CENTER_VERTICAL,
        )
        self.ground_check = wx.Choice(pane, size=(FIELD_W, -1), choices=GROUND_CHECKS)
        self.ground_check.SetSelection(0)
        grid.Add(self.ground_check, 0)
        self._add_fields(pane, grid, "ground")
        box.Add(grid, 0, wx.BOTTOM, theme.HAIR)
        return box

    def _markers_box(self, pane):
        """Markers: the User layer the plugin's marker footprints are drawn on
        -- one pick for the feed marker and the area markers alike, shared
        across the pages like the feed picks. The marker section owns the widget
        -- it is the one that places and reshapes the footprint, and it seeds
    ...
    def snapshot(self):
        ...
        settings): the optimization toggles, cells-across-driven-copper, the
        copper model, the ground check, the numeric ``adv.*`` knobs and the
        ...
        data["model"] = self.model.GetStringSelection()
        data["ground_check"] = self.ground_check.GetStringSelection()
    ...
    def restore(self, data):
        ...
        set_choice(self.model, data.get("model"))
        set_choice(self.ground_check, data.get("ground_check"))
        for key, ctrl in self.adv.items():
    ...
    def preset_index(self):
        ...
        copper_cells = to_float(self.copper_cells.GetValue(), None)
        if copper_cells is not None and copper_cells == int(copper_cells):
            copper_cells = int(copper_cells)
        state = (
            self.coarse_air.GetValue(),
            not self.no_refine_xy.GetValue(),
            "mur" if self.mur.GetValue() else "pml",
            self.conformal.GetValue(),
            self.adaptive.GetValue(),
            copper_cells,
        )
    ...
    def contribute(self, params):
        ...
        copper_cells = to_float(self.copper_cells.GetValue(), None)
        if (
            copper_cells is None
            or copper_cells != int(copper_cells)
            or not 1 <= copper_cells <= 5
        ):
            raise RuntimeError(
                f"Cells across driven copper: "
                f"'{self.copper_cells.GetValue()}' is not a whole number from "
                "1 to 5"
            )
        params.update(
            {
                "copper_model": MODELS[max(self.model.GetSelection(), 0)][0],
                # speed / quality toggles
                "coarse_air": self.coarse_air.GetValue(),
                "refine_xy": not self.no_refine_xy.GetValue(),
                "boundary": "mur" if self.mur.GetValue() else "pml",
                "conformal": self.conformal.GetValue(),
                "adaptive": self.adaptive.GetValue(),
                "copper_cells": int(copper_cells),
                "refine_adaptive": self.refine_adaptive.GetValue(),
                "feed_snap_to_center": self.feed_snap_to_center.GetValue(),
                "mesh_fit_cell": self.mesh_fit_cell.GetValue(),
                "mesh_nudge": self.mesh_nudge.GetValue(),
                "ground_check": self.ground_check.GetStringSelection(),
            }
        )
        # The Materials pickers emit the per-layer metal_layers /
        # substrate_layers lists config applies entry by entry. They are the
        # only source of conductor/dielectric loss (config invents none), so a
        # layer left without a material blocks the run in _apply_overrides.
        if self.materials is not None:
            self.materials.apply(params)
        for key, label, _hint in ADV_FIELDS:
            text = self.adv[key].GetValue().strip()
            if text:
                params[key] = _adv_value(key, label, text)
