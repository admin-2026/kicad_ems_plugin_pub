"""The Results box: what a run or a scan left behind, re-opened without redoing it.

One shape, two views. ``ResultsSection`` is the reusable base -- a titled box
holding one button per view it can open, over a status line that says why a view
isn't there yet. A subclass declares its buttons (``_VIEWS``: label, key, window
title, viewer slot) and what a view of each key would draw right now
(``_locate``); everything else -- building the row, the missing-view message, the
after-a-run auto-open -- is shared.

The solver writes data, never pages: what a run leaves is a ``pcb_*.js`` dump,
and what draws it is the viewer the plugin ships (sim.simulate.viewer_url,
antenna_plugin/viewer/). So ``_locate`` returns the *data* -- a dump, or a
scan's manifest naming several -- and ``present`` is where it becomes a page.

Two boxes subclass it:

* ``RunResultsSection`` (the simulate view) -- the newest archived grid/report
  dump of this board's runs (``results/<stamp>/``, sim.simulate.latest_result).
  The run section hands its freshly archived dumps to the same ``present``, so a
  result shown the moment the solver wrote it and one re-opened days later take
  one path: same window, same title.
* ``ScanResultsSection`` (the wizard) -- the last scan of this board, through
  the manifests it wrote into its scan folder (design.scan_views): one page,
  every candidate, a switcher between them.

Neither box knows how the data was produced, and neither producer knows how it
is shown: the scan/run sections only say "here is a result" or "there are new
