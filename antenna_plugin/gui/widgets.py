...
def no_scroll_tree(container):
    ...
    wrapped inline with no_scroll are skipped by its guard flag."""
...
class _WheelGuard:
    ...
    no_scroll already keeps the wheel off these controls there, and second-
    guessing native input we never see is how real edits get eaten.
...
# --------------------------------------------------------------------------- #
# widget <-> flat string state (shared by the sections' snapshot/restore and
# the persistence layer, settings.py -- both store the form as flat strings)
# --------------------------------------------------------------------------- #
def bool_str(value):
