"""Emit the runner YAML config the FDTD binary reads.

Self-contained port of the config half of ``ems/scripts/pcb_config.py`` --
kept inside the plugin package on purpose, so the plugin has no runtime
dependency on the ``ems`` tree (only the bundled binary). Instead of scanning
a directory, it takes the gerber paths already resolved by
``simulate.plot_gerbers`` plus the per-layer physical stackup (thickness_mm
per copper foil; thickness_mm, eps and loss_tangent per dielectric gap) read
off the board by ``simulate.collect_stackup``.

Stdlib only (writes YAML by hand). The runner's RunConfig loader
(``src/config/PcbRunConfig.cpp`` in the simulator) is the only reader and
understands exactly this schema; keep the two in step.
"""
...
# window reads the same module, see antenna_plugin/cellsize).
...
#
# What the schema asks for, in the order write_yaml lays it out:
#
#   * geometry -- only the board outline (`edge:`) and the two drill files are
#     top-level. Every other gerber rides on the stackup entry it belongs to,
#     so the runner reads each layer's artwork off the stack it meshes. Holes
#     need no key of their own: the plated drills become via columns, the
#     non-plated ones remove laminate and copper as the shapes they are (a
#     routed slot is a slot, not two round holes at its ends), and whatever is
#     drawn on the outline layer that is not a closed boundary loop -- where
#     KiCad puts a mounting slot or a cutout -- is material removed too. The
#     schema's optional `holes:` gerber is for boards that draw their cutouts
#     on a layer of their own, which KiCad does not have, so it is never
#     emitted here.
#   * stackup -- KiCad board-setup order, top to bottom. `copper` foils (each
#     with its `sigma`) and `core` gaps (`thickness_mm`, `eps`, `loss_tangent`)
#     are simulated; `paste` and `silk` are geometry-only overlays with no
#     thickness and no material. The paste apertures are the ground check's pad
#     signal -- a paste aperture on copper IS a pad -- and the solder mask is
#     not emitted at all, nothing in the runner reads it.
#   * mesh and run knobs -- every one has a runner-side auto, spelled 0, that
#     resolves from the config's own band, stackup and copper geometry.
#     DEFAULTS below emits those zeros, so an untouched Advanced field means
#     "auto" and only a typed value pins a knob. Materials are the deliberate
#     exception: there is no auto for a conductivity or a permittivity, and a
#     layer with no source blocks the run (see _apply_overrides).
#   * outputs -- a run writes data and nothing else, and always writes both
#     dumps (pcb_data.js, pcb_grid.js), so there is no key here for choosing
#     them. `output_json` asks for the data dump a second time as bare JSON,
#     which is a different file, not a different deliverable.
#   * feed port -- a point plus a direction vector, and nothing else: the
#     runner infers the driven trace from the copper under the point and severs
#     a one-cell gap on the simulation grid. The gap is therefore an OUTPUT of
#     meshing, not an input to it (the run reports what it built, MESH-026/027).
#
# Two knobs are conditional rather than always spelled out -- `adaptive_n` and
# the refine_n/refine_adaptive pair -- because the runner rejects each of them
# without the switch it grades; see write_yaml.
#
...
# A copper foil's sigma comes from the Materials picker (its own
# catalog default is Copper); a dielectric's eps and loss tangent come from the
# board's Physical Stackup (or a picked substrate material that overrides both).
# The silk/paste overlays need no material at all -- they are geometry-only
# (never meshed), see write_yaml. A layer with no source for a value is a
# blocked run (stackup_problems /
# _apply_overrides raise), never a silently injected number -- see the
...
# value only appears here when the user typed one
# into an Advanced field. The material values (`copper_sigma`,
# `substrate_sigma`, `substrate_eps`) are intentionally absent: they have no
# default at all -- each layer's conductivity/permittivity must come from the
# Materials picker or the board stackup, and a layer with no source blocks the
# run rather than taking an invented number (see _apply_overrides).
DEFAULTS = {
    "copper_model": "sheet",
    "conformal": True,
    "fpattern_ghz": 2.45,
    ...
    # (see gui/options.SPEED_PRESETS); off by default so a GUI run below the
    ...
    "output_json": False,
    "substrate_cell_mm": 0.0,  # 0 = auto: min(cell, thinnest slab / 4)
...
def write_yaml(gerbers, stack, params, yaml_path):
    """Write the runner config to ``yaml_path``.

    ``gerbers`` is the dict from plot_gerbers (resolved paths by role),
    ``stack`` the collect_stackup result (its ``stackup`` list drives the
    emitted geometry: each layer gets its gerber ``path`` folded in from
    ``gerbers`` -- see _layer_gerber -- plus any GUI material overrides, see
    _apply_overrides), ``params`` the GUI knobs. The single feed port comes from
    ``params['feed']`` -- a point + direction, from a placed feed marker
    (feed_marker.feed_dict) or a wizard (Geometry.feed_dict); a missing feed is
    an error. Returns the outdir written into the config.
    """
    import os
    ...
    outdir = p.get("outdir") or yaml_dir

    if not gerbers.get("edge") or not gerbers.get("copper"):
    ...
    # The cell and its ceiling are a range, and two ways of typing them are not
    # one: a start above the ceiling (no cell satisfies both) and a feature
    # floor above it (a floor that can never bind on any lattice this config
    # permits). The runner refuses both at config load; refuse them here, in
    # the same words, before a run is prepared -- the rule itself lives with
    # the knobs it is about (cellsize.knobs), which is also what the window
    # captions the pair with while it is being typed.
    clash = cellsize.problem(
        p["cell_mm"],
        p["cell_max_mm"],
        floors=(
            ("feature_min_cell_mm", p["feature_min_cell_mm"]),
            ("via_min_cell_mm", p["via_min_cell_mm"]),
        ),
    )
    if clash:
        raise RuntimeError(clash)

    stackup = _apply_overrides(stack.get("stackup") or [], p)

    lines = [
        f"config_version: {CONFIG_VERSION}",
        "# generated by antenna_plugin/sim/config.py",
        "",
        ...
        f"edge: {_relpath(gerbers['edge'], yaml_dir)}",
    ]
    for key in ("vias", "npth"):
        if gerbers.get(key):
            lines.append(_kv(key, _relpath(gerbers[key], yaml_dir)))

    lines += [
        "",
        "# --- physical stackup (KiCad board-setup order, top to bottom) ---",
        "# Each layer carries its own gerber `path` (copper/paste/silk); a core",
        "# gap is a bare dielectric with no gerber. copper/core entries are",
        ...
        "# informational. An overlay's side comes from its position here:",
        "# before the first copper entry is top, after it is bottom.",
        "stackup:",
    ]
    copper_paths = dict(gerbers.get("copper") or [])
    for entry in stackup:
        kind = entry["type"]
        path = _layer_gerber(entry, gerbers, copper_paths)
        if not path and kind not in ("copper", "core"):
            # An overlay whose layer wasn't plotted (silk while _PLOT_SILK is
            # off, or a board with no paste layer) tells the runner nothing --
            # leave it out rather than emit a gerber-less entry.
            continue
        prefix = "  - "
        for key in ("type", "name", "thickness_mm", "eps", "loss_tangent", "sigma"):
            ...
            prefix = "    "
        if path:
            lines.append(f"{prefix}path: {_relpath(path, yaml_dir)}")
            prefix = "    "
        # KiCad plots the paste layer positive -- the drawn regions ARE the
        # stencil apertures, i.e. the pads -- so no `is_negative` here.
        #
        # The grid dump carries every layer but the paste: its apertures sit on
        # the copper cells it marks, so drawing them only overdraws the foil.
        # The layer is here for the ground check, not for the view.
        lines.append(f"{prefix}show_in_grid: {_scalar(kind != 'paste')}")

    lines += [
        "",
        "# --- copper model ---",
        _kv("copper_model", p["copper_model"]),
        _kv("conformal", bool(p["conformal"])),
        "",
        "# --- simulation parameters ---",
        _kv("outdir", _relpath(outdir, yaml_dir)),
        "# 0 = auto: run until the port rings down (steps wins if both set)",
        _kv("time_ns", float(p["time_ns"])),
        _kv("steps", int(p["steps"])),
        _kv("threads", int(p["threads"])),
        _kv("fpattern_ghz", float(p["fpattern_ghz"])),
        _kv("fmax_ghz", float(p["fmax_ghz"])),
        _kv("flow_ghz", float(p["flow_ghz"])),
        _kv("port_resistance", p["port_resistance"]),
        _kv("cell_mm", float(p["cell_mm"])),
        ...
        _kv("cell_max_mm", float(p["cell_max_mm"])),
        "# N of the auto cell's copper term: cell <= driven copper / N",
        _kv("copper_cells", int(p["copper_cells"])),
        _kv("margin_mm", float(p["margin_mm"])),
        _kv("coarse_air", bool(p["coarse_air"])),
        _kv("air_cell_mm", float(p["air_cell_mm"])),
        _kv("mesh_ratio", float(p["mesh_ratio"])),
        ...
        _kv("mesh_budget_gb", float(p["mesh_budget_gb"])),
        ...
        _kv("mesh_fit_cell", bool(p["mesh_fit_cell"])),
        _kv("rotation_deg", float(p["rotation_deg"])),
        _kv("substrate_cell_mm", float(p["substrate_cell_mm"])),
        _kv("boundary", p["boundary"]),
        _kv("pml_cells", int(p["pml_cells"])),
        _kv("refine_xy", bool(p["refine_xy"])),
        _kv("feature_max_mm", float(p["feature_max_mm"])),
        _kv("feature_min_cell_mm", float(p["feature_min_cell_mm"])),
        _kv("via_min_cell_mm", float(p["via_min_cell_mm"])),
    ]
    if p["refine_xy"]:
        # refine_n / refine_adaptive act only through refine_xy -- the runner
        # rejects either key with refine_xy: false -- so emit them only when
        # it's on; an Advanced refine_n typed with refine_xy off is then
        # harmlessly ignored, same as adaptive_n below.
        lines.append(_kv("refine_n", int(p["refine_n"])))
        lines.append(_kv("refine_adaptive", bool(p["refine_adaptive"])))
    lines += [
        # Post-mesh nudging, unconditional -- unlike the refine knobs above it
        # needs nothing else turned on, and with refine_xy on it simply runs on
        # the refined mesh.
        ...
        _kv("mesh_nudge", bool(p["mesh_nudge"])),
        # Adaptive rastering. adaptive_n has no runner default,
        # and an adaptive_n without adaptive: true is a config error, so emit
        # the reach only when adaptive is on -- an Advanced adaptive_n typed
        # while the slider is off an adaptive stop is then harmlessly ignored.
        _kv("adaptive", bool(p["adaptive"])),
    ]
    if p["adaptive"]:
        lines.append(_kv("adaptive_n", float(p["adaptive_n"])))
    lines += [
        _kv("auto_ground", bool(p["auto_ground"])),
        "# 0 = derive the strap search's disk from the port (half the feed gap",
        "# + two run limits + a trace width)",
        _kv("auto_ground_radius_mm", float(p["auto_ground_radius_mm"])),
        _kv("ground_check", p["ground_check"]),
        "# floor for the ground check's copper-connectivity raster, not the mesh;",
        "# 0 = auto: min(0.1 mm, narrowest fed trace / 2)",
        _kv("min_raster_mm", float(p["min_raster_mm"])),
        "",
        "# --- outputs to write. Both dumps (pcb_data.js, pcb_grid.js) are",
        "# always written; output_json asks for the data one as bare JSON too.",
    ]
    lines.append(_kv("output_json", bool(p["output_json"])))
    lines.append("")
    lines += _feed_lines(gerbers, p)
...
def _feed_lines(gerbers, p):
    """The single ``feed_ports:`` entry, from ``params['feed']`` -- a point on
    the feed line plus the direction vector (dir_x/dir_y toward the antenna),
    gerber mm. That is the whole port: the runner infers the trace axis and
    width from the copper under the point and severs a one-cell gap on the
    simulation grid, so no clearance rect and no gap width is written. Both the
    placed feed marker (feed_marker.feed_dict) and the L-wizard
    (design.geometry.Geometry.feed_dict) produce this feed. The port-wide
    ``feed_snap_to_center`` rides just above the list: it decides whether the
    runner keeps that point or centers it on the copper it drives. ``layer`` is
    the copper foil the port sits on, by its stackup copper entry name (e.g.
    ``F_Cu``); defaults to the top foil."""
    feed = p.get("feed")
    if not feed:
        raise RuntimeError(
            "no feed specified: place a feed marker (Feed marker box) on the feed line"
        )
    layer = p.get("feed_layer") or gerbers["copper"][0][0]
    entry = [
        ("x_mm", float(feed["x"])),
        ("y_mm", float(feed["y"])),
        ("layer", layer),
        ("dir_x", float(feed["dir_x"])),
        ("dir_y", float(feed["dir_y"])),
    ]
    lines = [
        ...
        "copper either way)",
        _kv("feed_snap_to_center", bool(p["feed_snap_to_center"])),
        "feed_ports:",
    ]
    prefix = "  - "
